...
GOVCC_PKG_ID=$(peer lifecycle chaincode queryinstalled 2>/dev/null | grep "govcc_1:" | awk '{print $3}' | tr -d ',')
INVESTCC_PKG_ID=$(peer lifecycle chaincode queryinstalled 2>/dev/null | grep "investcc_1:" | awk '{print $3}' | tr -d ',')
